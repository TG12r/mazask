...
function record() {
    ...
    popUpContainer.classList.remove('hidden');
    $('#btn-guardar-record').click(function() {
        $.ajax({
            url: '/src/guardar-record.php', // El archivo PHP al que enviar la solicitud
            ...
            error: function(xhr, status, error) { // Función que se ejecuta cuando ocurre un error
...
function actualizarTemporizador() {
    ...
    minutos = Math.floor(tiempoTranscurrido / 60).toString().padStart(2, '0');
    segundos = (tiempoTranscurrido % 60).toString().padStart(2, '0');
